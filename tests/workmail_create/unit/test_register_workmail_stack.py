...
def test_register_workmail_stack_success(mock_rds_client):
    ...
    register_workmail_stack(
        OWNER_ID, EMAIL_USERNAME, VANITY_NAME, mock_rds_client, CONFIG
    )
    ...
    assert (
        kwargs["sql"]
        == """
        INSERT INTO workmail_stacks (ownerid, email_username, vanity_name)
        VALUES (:ownerid, :email_username, :vanity_name)
        """
    )
